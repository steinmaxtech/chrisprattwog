...
const AvatarIcon = () => (
  <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="22" cy="22" r="22" fill="#1c1009"/>
    <circle cx="22" cy="17" r="7" fill="#e97316"/>
    <ellipse cx="22" cy="35" rx="12" ry="8" fill="#e97316"/>
  </svg>
);
...
export default function App() {
  ...
  const [templateIdHistory, setTemplateIdHistory] = useState({});   // { LVL: ["103095", ...], ... }
    ...
    <div style={{ fontFamily: "'DM Mono','Courier New',monospace", background: T.bg, minHeight: "100vh", color: T.text }}>
      ...
      <div style={{ background: T.header, borderBottom: `2px solid ${T.accent}`, padding: "0 2rem" }}>
        <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", alignItems: "center", gap: 14, padding: "1.1rem 0" }}>
          <div style={{ width: 44, height: 44, borderRadius: "50%", overflow: "hidden", flexShrink: 0, border: `2px solid ${T.accent}` }}><AvatarIcon /></div>
          <div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "2rem" }}>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    {showTidDropdown && templateIdHistory[woType]?.length > 0 && (
                      <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: T.surface, border: `1px solid ${T.border2}`, borderRadius: 7, zIndex: 100, marginTop: 3, overflow: "hidden" }}>
                        {templateIdHistory[woType].map(tid => (
                          <div key={tid} onClick={() => { setWoConfig(prev => ({ ...prev, templateId: tid })); setShowTidDropdown(false); }}
                            style={{ padding: "8px 12px", cursor: "pointer", fontSize: 12, color: T.textMid, borderBottom: `1px solid ${T.border}` }}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24, gap: 12 }}>
          {step > 0
            ...
            : <div />}
          {step < 2 && (
            <button onClick={() => { if (step === 0) saveTemplateId(woType, woConfig.templateId); setStep(s => { setJoke(JOKES[Math.floor(Math.random() * JOKES.length)]); return s + 1; }); }} disabled={!canProceed[step]}
  ...
  return (
    <div style={{ fontFamily: "'DM Mono','Courier New',monospace", background: T.bg, minHeight: "100vh", color: T.text }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "2rem" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 24, gap: 12 }}>
          {step > 0
            ? <button onClick={() => setStep(s => s - 1)} style={{ background: T.btnBack, border: `1px solid ${T.border2}`, borderRadius: 8, padding: "10px 22px", color: T.textMid, cursor: "pointer", fontSize: 12, fontFamily: "inherit" }}>← Back</button>
            : <div />}
          {step < 2 && (
            <button onClick={() => { if (step === 0) saveTemplateId(woType, woConfig.templateId); setStep(s => s + 1); }} disabled={!canProceed[step]}
              style={{ background: canProceed[step] ? `linear-gradient(135deg,${T.accent},#dc6209)` : T.disabledBg, border: "none", borderRadius: 8, padding: "10px 28px", color: canProceed[step] ? "#000" : T.disabledText, cursor: canProceed[step] ? "pointer" : "not-allowed", fontFamily: "'Bebas Neue',sans-serif", fontSize: 16, letterSpacing: 2, boxShadow: canProceed[step] ? "0 2px 14px rgba(234,88,12,.4)" : "none", transition: "all .15s" }}>
              Continue →
            </button>
          )}
