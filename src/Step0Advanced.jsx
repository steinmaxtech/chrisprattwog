  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <div style={{ background: T.surface, borderRadius: 12, padding: "1.5rem", border: `1px solid ${T.border}` }}>
        ...
        {woType && (() => { const wot=ALL_WO_TYPES[woType]||{}; if (woType==="SDT") return <div style={{ fontSize:11, color:T.textFaint, marginTop:6 }}>10 work orders × 4 days · 3 technicians (fixed schedule)</div>; return <div style={{ fontSize:11, color:T.textFaint, marginTop:6 }}>{wot.numTechs} tech{wot.numTechs>1?"s":""} × {wot.numDays} day{wot.numDays>1?"s":""}</div>; })()}
      ...
      {woType === "SDT" && (
        <div style={{ background: T.surface, borderRadius: 12, padding: "1.5rem", border: `1px solid ${T.border}` }}>
          ...
          <div style={{ marginTop: 12, padding: "10px 12px", background: T.surface2, borderRadius: 7, fontSize: 11, color: T.textFaint, lineHeight: 1.7 }}>
            SDT generates a fixed <span style={{color:T.textMid}}>10-row, 4-day schedule</span> per site — 3 separate technicians, each bundled independently: <span style={{color:T.textMid}}>AH1</span> works all 4 days (Day 1 1pm–11pm, Days 2–4 11am–11pm), while <span style={{color:T.textMid}}>AH2</span> and <span style={{color:T.textMid}}>AH3</span> only work Days 2–4 (11am–11pm). Site IDs follow <span style={{color:T.textMid}}>xxxx-SDT-AH(#)</span>. Pay is Fixed per row ($400 Day 1, $650 Days 2–4) and isn't configurable here.
          </div>
        </div>
      )}
